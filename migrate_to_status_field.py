...
def verify_migration():
    ...
    if total_with_status == total_articles:
        ...
        print(f"⚠️  Warning: {total_articles - total_with_status} articles missing status field")
    
    # Show some example articles
    print(f"\n📝 Sample migrated articles:")
    sample_articles = Article.objects.all()[:5]
    for article in sample_articles:
        print(f"   - {article.id}: {article.status} | {article.title_it[:50] if article.title_it else 'No title'}...")
...
def cleanup_old_fields():
    ...
    response = input("⚠️  This will permanently remove 'validated' and 'used' fields. Continue? (yes/no): ")
