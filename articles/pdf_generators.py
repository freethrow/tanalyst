...
class ArticlesPDFGenerator:
    ...
    def generate_articles_pdf(self, articles: List[Article]) -> HttpResponse:
        ...
        def add_header_footer(canvas, doc):
            self._add_header_footer(canvas, doc, page_width, page_height, left_margin, right_margin)
    ...
    def _add_header_footer(self, canvas, doc, page_width, page_height, left_margin, right_margin):
        ...
        canvas.drawCentredString(page_width / 2, page_height - 1.5*cm, "TA - Technical Analyst")
        
        # Subtitle
        canvas.setFont('Helvetica', 10)
        metadata_text = f"Report Articoli | Generato il {datetime.now().strftime('%d/%m/%Y')}"
...
class WeeklySummaryPDFGenerator:
    ...
    def generate_weekly_summary_pdf(self, summary: WeeklySummary) -> HttpResponse:
        ...
        if main_trends:
            elements.append(Paragraph("Tendenze Principali", self.styles['section_title']))
        ...
        if featured_sectors:
            elements.append(Paragraph("Settori in Evidenza", self.styles['subsection_title']))
        ...
        if opportunities:
            elements.append(Paragraph("Opportunità per le Aziende Italiane", self.styles['section_title']))
