...
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, PageBreak, Table, TableStyle, BaseDocTemplate, PageTemplate, Frame, KeepTogether
...
def generate_pdf_report(request):
    ...
    column_width = (page_width - left_margin - right_margin - 0.5*cm) / 2  # 0.5cm gap between columns
    ...
    try:
        import os
        roboto_light_path = os.path.join(settings.BASE_DIR, 'static', 'fonts', 'Roboto-Light.ttf')
        manrope_bold_path = os.path.join(settings.BASE_DIR, 'static', 'fonts', 'Manrope-Bold.ttf')
    ...
    for idx, article in enumerate(articles, 1):
        ...
        # Add source link
        if article.url:
            source_link = f'<i>Fonte: <a href="{article.url}" color="grey">{article.url}</a></i>'
            try:
                article_elements.append(Paragraph(source_link, source_link_style))
            except:
                article_elements.append(Paragraph(f'<i>Fonte: {article.url}</i>', source_link_style))
    ...
    def add_header_footer(canvas, doc):
        ...
        canvas.rect(0, page_height - 3*cm, page_width, 3*cm, fill=True, stroke=False)
        # Header text in white
        canvas.setFont('Helvetica-Bold', 20)
        canvas.setFillColor(colors.white)
        canvas.drawCentredString(page_width / 2, page_height - 1.5*cm, "Business News Report")
        # Metadata in white
...
def generate_weekly_summary_pdf(request, summary_id):
    ...
    try:
        ...
        exec_table = Table(exec_data, colWidths=[page_width - left_margin - right_margin - 1*cm])
        ...
        if opportunities:
            elements.append(Paragraph("Opportunità per le Aziende Italiane", section_title_style))
            
            # Create highlighted box for opportunities
            opp_data = [[Paragraph(opportunities, highlight_box_style)]]
            opp_table = Table(opp_data, colWidths=[page_width - left_margin - right_margin - 1*cm])
        ...
        def add_header_footer(canvas, doc):
            ...
            canvas.setFillColor(colors.white)
            canvas.drawCentredString(page_width / 2, page_height - 1.3*cm, "Weekly Business Summary")
        ...
        frame = Frame(
            ...
            bottom_margin, 
            page_width - left_margin - right_margin, 
            page_height - top_margin - bottom_margin - 2.5*cm,  # Subtract header height
            id='normal'
        )
        ...
        response = HttpResponse(pdf_value, content_type='application/pdf')
        filename = f'weekly_summary_{summary_id}_{datetime.now().strftime("%Y%m%d")}.pdf'
        response['Content-Disposition'] = f'attachment; filename="{filename}"'
