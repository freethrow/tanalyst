...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🔄 Starting status field migration...')
        )
    ...
    def _verify_migration(self):
        ...
        if total_with_status == total_articles:
            self.stdout.write(
                self.style.SUCCESS("✅ All articles have status field!")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"⚠️  {total_articles - total_with_status} articles missing status")
            )
    ...
    def _cleanup_old_fields(self, dry_run=False):
        ...
        try:
            ...
            docs_with_validated = collection.count_documents({'validated': {'$exists': True}})
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✅ Cleaned up {result.modified_count} documents")
                )
                
                # Verify cleanup
                remaining_validated = collection.count_documents({'validated': {'$exists': True}})
                ...
                if remaining_validated == 0 and remaining_used == 0:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"⚠️  Some fields remain: validated={remaining_validated}, used={remaining_used}")
                    )
            else:
                self.stdout.write(
