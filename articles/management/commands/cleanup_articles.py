...
class Command(BaseCommand):
    help = (
        "Clean up articles collection: remove scraped_date field and set default status"
    )
    ...
    def handle(self, *args, **options):
        ...
        docs_with_scraped_date = collection.count_documents(
            {"scraped_date": {"$exists": True}}
        )
        self.stdout.write(
            f"Found {docs_with_scraped_date} documents with 'scraped_date' field"
        )
        ...
        if docs_with_scraped_date > 0:
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ Removed 'scraped_date' field from {result.modified_count} documents"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"Would remove 'scraped_date' field from {docs_with_scraped_date} documents"
                    )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS("✅ No documents found with 'scraped_date' field")
            )
        ...
        if docs_without_status > 0:
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ Set status to 'PENDING' for {result.modified_count} documents"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"Would set status to 'PENDING' for {docs_without_status} documents"
                    )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS("✅ All documents already have a status")
            )
        ...
        docs_with_status = collection.count_documents(
            {"status": {"$exists": True, "$ne": None, "$ne": ""}}
        )
        remaining_scraped_date = collection.count_documents(
            {"scraped_date": {"$exists": True}}
        )
        ...
        self.stdout.write(
            f"Documents with scraped_date field: {remaining_scraped_date}"
        )
